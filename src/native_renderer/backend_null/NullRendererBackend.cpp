...
void NullRendererBackend::SubmitPM4Packet(const PM4PacketInfo& packet) {
  ...
  REXLOG_INFO(
      ...
      packet.packet_offset, packet.payload_dword_count, packet.first_dword_count,
      packet.first_dwords[0], packet.first_dwords[1], packet.first_dwords[2],
      packet.first_dwords[3]);
}
...
void NullRendererBackend::SubmitPM4Draw(const PM4DrawInfo& draw) {
  ...
  REXLOG_INFO(
      ...
      draw.event_index, draw.opcode_name.empty() ? "PM4_DRAW" : draw.opcode_name,
      draw.packet_ptr, draw.buffer_ptr, draw.packet_offset, draw.index_count,
      ...
      draw.viz_query_condition, draw.vertex_shader_hash, draw.pixel_shader_hash);
}
...
void NullRendererBackend::SubmitPM4Constants(const PM4ConstantInfo& constants) {
  ...
  REXLOG_INFO(
      ...
      constants.event_index, constants.opcode, constants.packet_ptr, constants.address,
      constants.offset_type, constants.type, constants.index, constants.dword_count);
}
...
void NullRendererBackend::SubmitPM4Swap(const PM4SwapInfo& swap) {
  ...
  REXLOG_INFO(
      "BO2 native renderer CP swap #{} frontbuffer={:#010x} size={}x{} "
  ...
  switch (command.type) {
    case RenderCommandType::BindShader:
      REXLOG_INFO(
          ...
          "dwords={} guest={:#010x}",
          command.sequence, command.arg0, command.arg2, command.arg1, command.arg3,
          command.guest_address);
      break;
