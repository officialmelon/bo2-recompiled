...
constexpr uint32_t kCommandBuilder = 0x821DB3D8;
constexpr uint32_t kImpXamInputGetState = 0x826EACFC;
constexpr uint32_t kActiveClientPointer = 0x82908890;
constexpr uint32_t kPitchOffset = 7312;
constexpr uint32_t kYawOffset = 7316;
...
REX_HOOK_RAW(default_native_command_builder) {
  ...
  const uint32_t active_client = REX_LOAD_U32(kActiveClientPointer);
  if (!active_client) {
    return;
  }
...
REX_HOOK_RAW(default_native_input_tick) {
  ...
  while (bo2::NativeInput::PopKeyEvent(event)) {
    ...
    if (const uint16_t ui_key = UiCompatibilityKey(event.key)) {
      ...
      sub_821E06A8(ui_frame.ctx, base);
    }
  }
  ctx.r3.u64 = result;
}
...
void InstallDefaultNativeInput(rex::Runtime* runtime) {
  ...
  original_command_builder =
      InstallHostDetourWithTrampoline(&sub_821DB3D8, &default_native_command_builder);
  if (!original_command_builder) {
#if defined(__ANDROID__)
    original_command_builder = &sub_821DB3D8;
#else
    return;
#endif
  }
  runtime->function_dispatcher()->SetFunction(kCommandBuilder,
                                               &default_native_command_builder);
  runtime->function_dispatcher()->SetFunction(kImpXamInputGetState,
                                               &default_native_input_tick);
  InstallHostDetour(&__imp__XamInputGetState, &default_native_input_tick);
