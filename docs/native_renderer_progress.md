...
- Offline real D3D12 replay can bind decoded captured BO2 vertex/index data for draw `1004`/`1209`, submit `DrawIndexedInstanced`, copy the target to CPU memory, and write a BMP. Draw `1209` can use a hash-keyed manual HLSL override pair; the temporary diagnostic shader still exists only behind `--allow-diagnostic-shader`.
...
- There is a first D3D12 resource-backed replay path that submits all `11` captured BO2 indexed draws for one supported shader pair with real vertex/index buffers, flattened captured constants, and a manual HLSL override pair, but it does not yet translate Xenos shaders, reconstruct full constant layouts, bind captured textures, or replay render-target/depth state.
...
- Replay currently has draw/index metadata, constant payloads, bounded index snapshots, vertex fetch records, bounded vertex snapshots, bounded texture fetch snapshots, decoded render-state fields, and CPU-side vertex component decoding for observed formats. It still lacks sidecar manifests, complete texture decode/upload, native sampler binding, render-target/depth resource snapshots, and backend application of blend/raster/depth state.
...
4. Decode and upload the captured texture snapshots for draw `799`, including Xenos tiled format `6` and sampler state, then bind them in D3D12 replay.
