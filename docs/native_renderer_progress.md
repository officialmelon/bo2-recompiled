...
- `native_shader_inspect.exe` now also inspects extracted shader container and microcode files directly with `--shader --dump-header`, `--microcode --dump-words`, and `--microcode --disassemble`. The current "disassemble" output is a raw unknown-preserving big-endian dword listing with recovered embedded `pimp_technique_*` / `pimp_shader_*` strings; true Xenos opcode decode and shader IR generation remain future work.
...
- `native_shader_inspect.exe` can inspect extracted shader containers and `.ucode` files without loading a full shader index, including stage-guess header fields, descriptor size candidates, embedded shader/technique strings, big-endian dword dumps, and an explicit unknown-preserving raw listing for decoder bring-up.
