...
- Runtime shader payload semantic decode and diagnostic HLSL/DXBC generation now exist for captured PM4 payloads, but static extracted `.ucode` semantic decode is not solved because those files include extracted metadata/constants before the analyzer's expected payload range. Raw static `.ucode` listing and raw IR still work, and the generated HLSL remains diagnostic until decoded Xenos operations are lowered.
