...
- Multi-pair frame replay update: D3D12 frame skip replay now builds a small PSO cache keyed by runtime VS/PS pair, preserves prepared draw order, switches root signature/PSO per draw, and submits every supported draw whose pipeline can be created. On `vertex_recapture_001`, `--frame 0 --backend d3d12 --skip-unsupported` now submits all `260/260` geometry-supported real BO2-derived draws across `7` shader pairs: `120` for `1E6883/A4A965`, `65` for `5B9B/3A68`, `4` for `5D918D/C4ED`, `26` for `81311A/246E`, `26` for `AB1E86/246E`, `15` for `AB1E86/A4A965`, and `4` for `AB1E86/C4ED`. It logs `PSO cache: entries=10 misses=10 hits=250`, binds `240` captured texture SRVs/samplers, and writes `native-renderer-pair-frame0-d3d12.bmp`.
...
- Vertexless frame regression: `--frame 0 --backend d3d12 --skip-unsupported` exits `0`, reports `frame_draws=2415`, `geometry_supported=2350`, `noop_elided=3`, `skipped=62`, and submits all `2350` supported draws across `8` shader pairs. The remaining skips are `62 x DDED7E no-fetch point shader needs Xenos r0/register initialization semantics before D3D12 can draw it`. The run binds `240` captured texture SRVs, `240` captured sampler descriptors, reports `0` texture decode fallbacks, and writes `native-renderer-dded-skip-d3d12.bmp`.
- Vertexless strict regression: `--frame 0 --backend d3d12` now elides the three no-side-effect `DDED7E/A4A965` no-fetch draws and fails later at frame `0` draw `1967` event `1968` with `VS=0xDDED7E538422AE73 PS=0x3A6876055FEC1674: DDED7E no-fetch point shader needs Xenos r0/register initialization semantics before D3D12 can draw it`; it no longer fails at draw `0` or at the color-mask-zero `DDED7E/A4A965` draw.
