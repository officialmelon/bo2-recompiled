...
- DSV binding update: the D3D12 real replay path now creates an offscreen `D32_FLOAT` depth resource, creates a DSV heap/view, declares `DSVFormat=DXGI_FORMAT_D32_FLOAT` on real replay PSOs, binds the DSV for real draws, and clears it before drawing. This is the first DSV binding step; it is not yet a captured BO2 depth target because target readback/resolve data is still not available.
