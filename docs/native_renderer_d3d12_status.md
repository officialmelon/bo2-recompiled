...
- Remaining limitation: these SRVs and sampler descriptors are now real captured resources/state in the command stream, and the first PSO-side render-state subset is applied. The backend still lacks additional texture formats/mips, render-target/depth resource snapshots, real DSV binding for depth-enabled draws, per-state PSO switching across heterogeneous draws, and full blend/depth/stencil coverage.
