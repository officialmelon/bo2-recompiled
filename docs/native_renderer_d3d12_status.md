...
- Vertexless frame regression: `--frame 0 --backend d3d12 --skip-unsupported` exits `0`, reports `frame_draws=2415`, `geometry_supported=2350`, `noop_elided=3`, `skipped=62`, and submits all `2350` supported draws across `8` shader pairs. The remaining skips are `62 x draw has no captured vertex/fetch state`, now headed by the enabled-color `VS=0xDDED7E538422AE73` / `PS=0x3A6876055FEC1674` point-draw class. The run binds `240` captured texture SRVs, `240` captured sampler descriptors, reports `0` texture decode fallbacks, and writes `native-renderer-noop-elide-frame0-d3d12.bmp`.
- Vertexless strict regression: `--frame 0 --backend d3d12` now elides the three no-side-effect `DDED7E/A4A965` no-fetch draws and fails later at frame `0` draw `1967` event `1968` with `VS=0xDDED7E538422AE73 PS=0x3A6876055FEC1674: draw has no captured vertex/fetch state`; it no longer fails at draw `0` or at the color-mask-zero `DDED7E/A4A965` draw.
