...
- Frame replay planning update: `native_render_replay.exe --capture native_captures\vertex_recapture_001\events.jsonl --frame 0 --backend d3d12` now fails closed by default with the first unsupported draw and reason. On the current capture it exits `1` with `D3D12 frame replay strict failure at frame 0 draw 0 event 1 ... draw has no captured vertex/fetch state`.
- Frame skip replay update: `--frame 0 --backend d3d12 --skip-unsupported` now plans the frame, groups unsupported geometry reasons, and renders the first supported shader-pair batch. Because `vertex_recapture_001` has frame markers but no frame-owned PM4 draws (`46` frames, `2415` draws all classified pre-frame), the backend explicitly uses the pre-frame draw bucket for this capture shape. The run reports `frame_draws=2415`, `geometry_supported=260`, `skipped=2155`, `rendered_current_shader_pair=120`, and writes `native-renderer-frame0-skip-d3d12.bmp`.
- Multi-pair frame replay update: D3D12 frame skip replay now builds a small PSO cache keyed by runtime VS/PS pair, preserves prepared draw order, switches root signature/PSO per draw, and submits every supported draw whose pipeline can be created. On `vertex_recapture_001`, `--frame 0 --backend d3d12 --skip-unsupported` submits `230` real BO2-derived draws across `5` shader pairs: `120` for `1E6883/A4A965`, `65` for `5B9B/3A68`, `4` for `5D918D/C4ED`, `26` for `81311A/246E`, and `15` for `AB1E86/A4A965`. It logs `PSO cache: entries=5 misses=5 hits=225` and writes `native-renderer-pso-frame0-skip-d3d12.bmp`.
- Render-state PSO-key update: the D3D12 real replay PSO cache key now includes runtime VS/PS hashes, expanded primitive topology, render target format, captured depth/color formats, blend/depth/stencil/raster registers, decoded depth/stencil/cull/fill/front-face flags, color mask, and MSAA sample count. On the same frame-skip replay, submitted draw coverage remains `230` draws across `5` shader pairs, while PSO cache entries increase to `8` (`misses=8`, `hits=222`), proving same-pair draws are now split by captured state where required. Output: `native-renderer-statekey-frame0-skip-d3d12.bmp`.
...
- DSV/depth-state update: the D3D12 real replay path now creates an offscreen `D24_UNORM_S8_UINT` depth/stencil resource, creates a DSV heap/view, declares `DSVFormat=DXGI_FORMAT_D24_UNORM_S8_UINT` on real replay PSOs, binds the DSV for real draws, and clears depth plus stencil before drawing. Captured `depth_test_enable`, `depth_write_enable`, `depth_func`, stencil enable, stencil masks, and stencil ref now feed the real PSO/stencil state. Stencil ops are still conservative `KEEP` operations until the remaining Xenos stencil-op fields are decoded.
...
- DSV frame regression result: `--frame 0 --backend d3d12 --skip-unsupported` exits `0`, submits the same `230` supported draws across `5` shader pairs, binds `120` captured texture SRVs and `120` captured samplers, and reports `depth_enabled_draws=82 depth_write_draws=82 stencil_enabled_draws=120`.
...
- Current frame-mode limitation: the D3D12 frame path now switches between multiple shader pairs, captured-state PSO variants, and dynamic input-layout variants, but it still uses a single offscreen RGBA8 render target and a synthetic depth target. The next required expansion is real render/depth target descriptors, captured depth contents, richer input/resource format mapping, and corrected frame-boundary sequencing so PM4 draws are assigned to real begin/end frame ranges instead of the pre-frame bucket.
