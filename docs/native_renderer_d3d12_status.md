...
- Build wrapper update: use `scripts\windows\build_native_d3d12.ps1` for native renderer iteration. It forces Visual Studio's CMake and Ninja onto `PATH`, avoiding MSYS2 CMake's broken Windows-path `file(GLOB)` results and the MinGW Ninja/Clang selection that caused long silent `default` rebuilds. Verified commands: `powershell -ExecutionPolicy Bypass -File scripts\windows\build_native_d3d12.ps1 -Target native_render_replay -Jobs 1` and the live D3D12 object target both exit `0`.
