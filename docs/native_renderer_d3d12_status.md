...
- Current frame-mode limitation: the D3D12 frame path now switches between multiple shader pairs/PSOs, but the PSO key is still only the shader pair and uses the first draw's render state for each pair. The next required expansion is a full PSO key including input layout, render/depth formats, blend/depth/stencil/raster state, topology class, and shader cache keys. The capture-boundary issue also needs follow-up so PM4 draws are assigned to real begin/end frame ranges instead of the pre-frame bucket.
