...
- Current frame-mode limitation: the D3D12 frame path now switches between multiple shader pairs, captured-state PSO variants, dynamic input-layout variants, and sequence-inferred frame buckets for the current pre-frame capture shape. Offline real replay now creates a small RTV cache keyed by captured guest color base and binds the matching target per submitted draw, but the presented target is still selected by replay-side heuristic and the depth target is still a synthetic replay DSV. The next required expansion is swap/resolve-proven presented-target selection, multiple MRT slots, real depth contents, richer input/resource format mapping, and live capture-boundary integration so PM4 execution and presentation are recorded on the same timeline.
- Input layout/resource checkpoint: the D3D12 real replay PSO key now includes a stable captured input-layout signature derived from vertex fetch stride, endian mode, and every captured attribute's format/offset/stride/sign/integer metadata. This prevents heterogeneous BO2 vertex fetch layouts from sharing a PSO simply because they lower into the same canonical `RealReplayVertex` mask. Replay output now reports `input_layout_variants` beside the PSO cache counters. Texture decode also accepts Xenos format `7` (`2_10_10_10`) and runtime-missing formats `23` (`24_8_FLOAT`) and `54` (`2_10_10_10_AS_16_16_16_16`) as strict captured RGBA8 upload previews; format `23` is currently a depth/stencil grayscale preview, so shader-exact depth compare semantics still need real typed handling.
- Input layout/resource validation: a fresh MSVC tooling build under `default\out\build\win-msvc-native-renderer-debug` builds `native_render_replay.exe` with the updated D3D12 replay source. `vertex_recapture_001 --validate --no-summary` exits `0`. Frame `3` strict real replay with `--skip-unsupported` exits `0`, submits `26` real BO2-derived draws across `3` shader pairs, and reports `PSO cache: entries=3 misses=3 hits=23 diagnostic_pipelines=0 cache_index_writes=3 input_layout_variants=3`. Resource-validation replay on `swap_fetch_capture_001` with explicit `--allow-diagnostic-shader` exits `0`, submits `7/7` draws, binds `28` captured texture SRVs and `28` captured samplers, and reports `unsupported_texture_attempts=0`.
...
- Live sequencing checkpoint: the live D3D12 backend now carries PM4 packets/draws/shader uploads/constants observed before `VdSwap` as pending work for the next native frame, instead of treating the `BeginFrame`/`EndFrame` pair around `VdSwap` as an empty frame. This matches the replay-side sequence-inferred frame bucket direction and gives live D3D12 integration a real per-frame command count to consume next.
- Live command infrastructure checkpoint: the live D3D12 backend now creates a direct command allocator, command list, and fence/event, resets the command list at `BeginFrame`, closes/executes it at `EndFrame`, and waits through the fence. The submitted command list is intentionally empty until real replay translation is factored into live mode; no diagnostic geometry or silent fallback is used.
...
- Build wrapper update: use `scripts\windows\build_native_d3d12.ps1` for native renderer iteration. It now emits and runs a temporary batch file so `VsDevCmd`, Visual Studio CMake/Ninja `PATH` setup, Ninja execution, and error-level propagation happen in one command shell. This avoids the earlier false-success path where a target command returned `0` without showing Ninja output. Verified command: `powershell -ExecutionPolicy Bypass -File scripts\windows\build_native_d3d12.ps1 -Configuration win-msvc-native-renderer-debug -Target native_render_replay -Jobs 1` exits `0` and prints Ninja status.
