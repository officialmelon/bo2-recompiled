...
- Vertexless frame regression: `--frame 0 --backend d3d12 --skip-unsupported` exits `0`, reports `frame_draws=2415`, `geometry_supported=2350`, `skipped=65`, and submits all `2350` supported draws across `8` shader pairs. The remaining skips are `65 x draw has no captured vertex/fetch state`, now headed by `VS=0xDDED7E538422AE73` / `PS=0xA4A965C189287B99`. The run binds `240` captured texture SRVs, `240` captured sampler descriptors, reports `0` texture decode fallbacks, and writes `native-renderer-vertexless-frame0-d3d12.bmp`.
- Vertexless strict regression: `--frame 0 --backend d3d12` now fails later at frame `0` draw `1965` event `1966` with `VS=0xDDED7E538422AE73 PS=0xA4A965C189287B99: draw has no captured vertex/fetch state`; it no longer fails at draw `0`.
