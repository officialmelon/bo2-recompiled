...
- Current frame-mode limitation: frame replay is still limited to one shader-pair batch after planning. Rendering every supported draw in a heterogeneous frame requires per-draw PSO/root-state switching and shader-pair grouping inside the D3D12 command list. The capture-boundary issue also needs follow-up so PM4 draws are assigned to real begin/end frame ranges instead of the pre-frame bucket.
