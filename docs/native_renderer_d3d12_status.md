...
- Current frame-mode limitation: the D3D12 frame path now switches between multiple shader pairs and captured-state PSO variants, but it still uses a canonical fixed input layout and single offscreen RGBA8 render target. The next required expansion is input-layout/resource-format keying, real render/depth target descriptors, DSV binding for depth-enabled draws, and shader cache keys in the PSO key. The capture-boundary issue also needs follow-up so PM4 draws are assigned to real begin/end frame ranges instead of the pre-frame bucket.
