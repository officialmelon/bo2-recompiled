...
- `xenos_limited_semantic_v3` now uses that typed real-resource PS evidence to generate cacheable HLSL for PS `0xC4ED2979F29C9139`. This is not a complete instruction-by-instruction PS translator yet: it recognizes the exact four-`tfetch2D`/export pattern and lowers it through the D3D12 replay backend's current single `t0/s0` texture binding until per-fetch SRV tables exist.
- D3D12 replay on `sidecar_capture_002` now succeeds with an empty override root and generated cache records for both `VS=0x5D918D91043B3ED0` and `PS=0xC4ED2979F29C9139`, submitting `5/5` supported draws and writing `native-renderer-generated-v3-pair-d3d12-real.bmp` with SHA-256 `6C10E0294634A70F7B465C8DF951875A65717920F8320498E139933DE4E34421`.
