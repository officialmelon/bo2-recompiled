...
- D3D12 real replay now uploads decodable captured texture fetches as `R8G8B8A8_UNORM` preview/upload textures, creates SRVs, and binds per-draw sampler descriptors from captured texture-filter and clamp fields. The decoded texture subset covers formats `2` (`k_8`), `6` (`k_8_8_8_8`), `18` (`DXT1`), `19` (`DXT2_3`), `20` (`DXT4_5`), `26` (`16_16_16_16`), `38` (`32_32_32_32_FLOAT`), and `49` (`DXN`). Unsupported texture-bearing draws fail closed in strict real mode; explicit diagnostic-shader mode may still bind diagnostic shader code for resource validation. Old captures without clamp fields continue to use clamp-addressing fallback.
- D3D12 real replay now consumes captured render state when creating replay PSOs: rasterizer culling/front-face/fill/depth-clip bits, color write mask, blend factors/ops, depth test/write flags, depth compare, stencil enable, stencil masks/ref, and bounded screen scissor. The real replay path now creates and binds an offscreen `D24_UNORM_S8_UINT` DSV so depth/stencil-enabled PSOs have a compatible target. Captured BO2 depth contents are still missing because target/depth readback sidecars are zero in the tested captures, and stencil ops are currently conservative `KEEP` operations until the remaining Xenos fields are decoded.
...
- Block-compressed verification: on `native_captures\swap_fetch_capture_001\events.jsonl`, the explicit diagnostic-shader resource validation command binds all `28/28` DXT4/5 texture fetches for the first supported shader pair as captured SRVs with `0` texture fallbacks and `0` unsupported texture attempts. The same capture without the explicit diagnostic-shader flag still fails closed at the missing native shader pair before resource fallback.
...
It still cannot produce real BO2 scene output because it lacks automatic translated BO2 shaders for all runtime pairs, complete Xenos texture format/mip coverage, nonzero captured render-target/depth resource snapshots, exact stencil-op translation, and corrected full-frame state sequencing. The visible D3D12 real output is therefore captured BO2 geometry through the current translated/override subset, not shader-correct full-scene rendering.
