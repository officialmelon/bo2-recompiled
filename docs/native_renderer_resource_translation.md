...
1. Replace bounded draw-time color/depth target previews with a proven GPU-side target readback/resolve path. CPU memory previews from target base addresses are implemented but zero for the tested capture.
