...
- Missing: automatic Xenos shader translation, DXC/DXIL compilation, full constant-layout reconstruction, render-target/depth resource snapshots, full Xenos texture tiling/format coverage, full sampler LOD/mip validation, real DSV binding for depth-enabled draws, per-state PSO switching across heterogeneous draws, and full-frame multi-draw replay.
