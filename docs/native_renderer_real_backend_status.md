...
Neither path is full BO2 scene rendering yet. The `d3d12` path proves native D3D12 vertex/index buffer binding, manual override shader manifest parsing, compiled shader-cache hits, flattened captured constant root binding, captured format-6 texture SRV binding, captured texture-filter sampler descriptors, first-pass captured PSO-side render-state setup, and `DrawIndexedInstanced` with captured BO2 geometry, but it still lacks automatic translated BO2 shaders, full constant-layout reconstruction, exact sampler clamp capture, render-target/depth resources, and full-frame state replay. Without a matching cache entry, override, or translated shader, real `d3d12` fails closed.
...
- Missing: automatic Xenos shader translation, DXC/DXIL compilation, full constant-layout reconstruction, render-target/depth resource snapshots, full Xenos texture tiling/format coverage, serialized sampler clamp modes, real DSV binding for depth-enabled draws, per-state PSO switching across heterogeneous draws, and full-frame multi-draw replay.
...
2. Serialize Xenos texture clamp modes and map them to D3D12 sampler addressing; filter descriptors are already dynamic per draw.
