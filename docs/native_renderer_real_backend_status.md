...
- Capture-boundary note: this capture has `46` frame markers, but every PM4 draw is currently classified before those markers. Frame `0` therefore uses an explicitly logged pre-frame draw bucket fallback for this capture only. The next capture/replay targets are assigning PM4 draws to actual frame ranges and expanding the PSO key beyond shader pair to include full render/depth/blend/raster/input-layout state.
