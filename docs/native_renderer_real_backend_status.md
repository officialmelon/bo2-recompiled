...
2. Expand captured render-state handling: draw-time color/depth target sidecars, real target descriptors, real DSV binding for depth-enabled draws, per-draw/per-state PSO switching, and complete stencil/blend coverage.
