...
- Multi-pair skip result: exit `0`, `frame_draws=2415`, `geometry_supported=260`, `skipped=2185`, `submitted_supported_draws=230`, output `native-renderer-pso-frame0-skip-d3d12.bmp`. The replay submits real BO2-derived draws across `5` shader pairs: `1E6883/A4A965`, `5B9B/3A68`, `5D918D/C4ED`, `81311A/246E`, and `AB1E86/A4A965`. Unsupported PSO creation is grouped by shader pair: `26` draws for `AB1E86/246E` and `4` draws for `AB1E86/C4ED` fail `CreateGraphicsPipelineState` with `0x80070057`.
...
- Capture-boundary note: this capture has `46` frame markers, but every PM4 draw is currently classified before those markers. Frame `0` therefore uses an explicitly logged pre-frame draw bucket fallback for this capture only. The next capture/replay targets are assigning PM4 draws to actual frame ranges and expanding beyond the canonical input layout/single offscreen target into real render/depth descriptors and DSV binding.
